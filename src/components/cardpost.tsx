...
export function CardPost({author, createdAt, subtitle, title, urlImage, slug}: CardPostProps) {
    ...
    return (
        <Link href={`/post/${slug}`} className="w-full sm:max-w-[352px] h-full flex flex-col items-center justify-between gap-2 sm:gap-4 hover:brightness-75 transition-all">
            <div className="flex w-full h-[200px] sm:h-[334px] relative rounded-2xl overflow-hidden">
                <Image
                    alt="imagem"
                    src={urlImage}
                    fill={true}
            ...
            <div className="flex w-full flex-1 flex-col justify-between gap-1 sm:gap-2">
                ...
                <div>
                    ...
                    <p className="text-zinc-600 text-sm text-xs md:text-sm">{format(new Date(createdAt), "dd 'de' MMMM 'de' yyyy", {locale: ptBR})}</p>
                </div>
            </div>
        </Link>
    )
